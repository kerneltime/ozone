...
implements: [I-9, I-11, I-8, I-13, I-2, I-mixed-shared-lock]
tests: [T-cross-thread-release, T-3, T-7, T-hot-stripe, T-holder-lease-negative]
anti_patterns:
  - "MUST be the single shared lock authority both paths acquire during mixed mode (D-17 I-mixed-shared-lock); the legacy path adopts these granular locks (#7583 integration-point-3). MUST NOT use the thread-affine OzoneManagerLock as the cross-model rendezvous — it cannot release on the continuation thread (I-9) and is released before the durable write."
...
implements: [I-determinism-followers-pure, I-apply-failure-resync, I-txninfo-atomic-with-patch, I-mixed-mode-cache-coherent, I-mixed-write-order]
...
anti_patterns:
  - "MUST NOT apply a migrated patch without (a) routing its durable write through the shared double-buffer drain during mixed mode (D-17 I-mixed-write-order) so it cannot reorder ahead of a queued legacy write, and (b) keeping the cache coherent on every node — invalidate written PartialTableCache keys, update the authoritative FullTableCache volume/bucket (D-17 I-mixed-mode-cache-coherent) — else cross-model reads go stale or durable writes reorder."
...
The three D-17 mixed-mode coherence axes retire here as well: with the legacy path gone there is
no cross-model race left, so the shared lock manager becomes sole-native (`I-mixed-shared-lock`),
the table cache is removed so its coherence upkeep goes away (`I-mixed-mode-cache-coherent`), and
the migrated path leaves the shared drain for its end-state direct write (`I-mixed-write-order`) —
all scoped P-0→P-7/P-8, restoring D-3.
